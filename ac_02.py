...
def test_02a():
    ...
    for data, expected in tests:
        result = calc_02a(data)
        assert result == expected, f"calc_02a({data}): Expected {expected} but got {result}"


def calc_02b(data):
    for i in range(len(data)):
        for j in range(i+1, len(data)):
            mx = max(data[i], data[j])
            mi = min(data[i], data[j])
            if mx % mi == 0:
                return mx / mi
    assert False, "No even divisible numbers found in %s" % str(data)
...
def test_02b():
    ...
    for data, expected in tests:
        result = calc_02b(data)
        assert result == expected, f"calc_02b({data}): Expected {expected} but got {result}"
...
test_02a()
print("02: %d" % sum(calc_02a(data) for data in puzzle_input_02))
test_02b()
print("02: %d" % sum(calc_02b(data) for data in puzzle_input_02))
