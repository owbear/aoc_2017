...
def test_04a():
    ...
    for data, expected in tests:
        result = calc_is_valid_04a(data)
        assert result == expected, f"calc_is_valid_04a({data}): Expected {expected} but got {result}"


def is_anagram_of(a, b):
    return tuple(a) in list(itertools.permutations(b))


def calc_is_valid_04b(data):
    seen = set()
    for w in data.split():
        for s in seen:
            if is_anagram_of(w, s):
                return False
        seen.add(w)
    return True
...
test_04a()
print("04a: %s" % sum(calc_is_valid_04a(l) for l in puzzle_input_04.readlines()))
puzzle_input_04.seek(0)
test_04b()
print("04b: %s" % sum(calc_is_valid_04b(l) for l in puzzle_input_04.readlines()))
